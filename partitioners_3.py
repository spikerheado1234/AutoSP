...
def solve_min_cut(
    ...
    # ===== Enhanced sequence-aware optimization =====
    # This implementation leverages pre-marked nodes (e.g., MLP layers marked with MUST_RECOMPUTE)
    # to make smarter decisions about memory vs compute tradeoffs:
    # 
    # 1. Respects explicit recomputation policies (MUST_RECOMPUTE, MUST_SAVE)
    # 2. Identifies patterns (MLP vs Attention) based on marked nodes
    # 3. Computes enhanced distances considering proximity to marked nodes
    # 4. Assigns weights that encourage MLP recomputation and attention saving
    # 5. Makes smart decisions for unmarked nodes based on their relationships
    #
    # Key insight: MLP layers are memory-heavy but computationally cheap,
    # while attention is computationally expensive (O(n²)) but memory-efficient
    #
    # Collect nodes already marked for recomputation (e.g., MLP layers)
    marked_recompute_nodes = set()
    marked_save_nodes = set()
    for node in joint_graph.nodes:
        if node.meta.get("recompute") == CheckpointPolicy.MUST_RECOMPUTE:
            marked_recompute_nodes.add(node)
        elif node.meta.get("recompute") == CheckpointPolicy.MUST_SAVE:
            marked_save_nodes.add(node)
    
    if AOT_PARTITIONER_DEBUG and marked_recompute_nodes:
        log.info(f"Found {len(marked_recompute_nodes)} nodes marked for recomputation")
        log.info(f"Found {len(marked_save_nodes)} nodes marked for saving")
    
    # Identify patterns: MLP vs Attention based on marked nodes
    def is_likely_mlp_node(node) -> bool:
        """Check if node is likely part of MLP based on marked patterns"""
        if node in marked_recompute_nodes:
            return True
        # Check if node feeds into or from marked MLP nodes
        for user in node.users:
            if user in marked_recompute_nodes:
                return True
        for arg in node.args:
            if isinstance(arg, fx.Node) and arg in marked_recompute_nodes:
                return True
        return False
    
    def is_likely_attention_node(node) -> bool:
        """Check if node is likely part of attention based on operations"""
        target = get_aten_target(node)
        if target in [aten.bmm.default, aten.softmax.int, aten._softmax.default]:
            return True
        # Check tensor shape for attention patterns
        if "val" in node.meta and isinstance(node.meta["val"], torch.Tensor):
            val = node.meta["val"]
            if val.dim() == 4:  # [batch, heads, seq, seq] pattern
                return True
        return False
    
    # Enhanced distance computation considering marked nodes
    def compute_enhanced_distance(node) -> int:
        """Compute distance that considers marked recomputation patterns"""
        base_distance = node.dist_from_bw
        
        # If node is marked for recomputation, reduce effective distance
        # This makes it more likely to be selected for recomputation
        if node in marked_recompute_nodes:
            return max(1, base_distance // 10)
        
        # If node is marked for saving, increase effective distance
        if node in marked_save_nodes:
            return min(1000, base_distance * 10)
        
        # Adjust distance based on proximity to marked nodes
        min_dist_to_recompute = float('inf')
        min_dist_to_save = float('inf')
        
        # BFS to find distance to nearest marked nodes
        visited = set()
        queue = [(node, 0)]
        while queue and len(visited) < 20:  # Limit search depth
            current, dist = queue.pop(0)
            if current in visited:
                continue
            visited.add(current)
            
            if current in marked_recompute_nodes:
                min_dist_to_recompute = min(min_dist_to_recompute, dist)
            if current in marked_save_nodes:
                min_dist_to_save = min(min_dist_to_save, dist)
            ...
            for arg in current.args:
                if isinstance(arg, fx.Node) and arg not in visited:
                    queue.append((arg, dist + 1))
        
        # Adjust distance based on proximity
        if min_dist_to_recompute < min_dist_to_save:
            # Closer to recompute nodes, reduce distance
            return max(1, base_distance - min_dist_to_recompute * 5)
        elif min_dist_to_save < min_dist_to_recompute:
            # Closer to save nodes, increase distance
            return min(1000, base_distance + min_dist_to_save * 5)
    ...
    def should_ban_recomputation(node):
        ...
        # Respect explicit recomputation policies
        if node.meta.get("recompute") == CheckpointPolicy.MUST_SAVE:
            return True  # Must save, ban recomputation
        if node.meta.get("recompute") == CheckpointPolicy.MUST_RECOMPUTE:
            return False  # Must recompute, allow it
        ...
        # ===== Smart decisions based on marked patterns =====
        # If this is likely part of MLP (based on marked nodes), prefer recomputation
        if is_likely_mlp_node(node):
            # Still ban if it's a random op
            if op_types.is_random(node):
                return True
            # Allow recomputation for MLP nodes
            if AOT_PARTITIONER_DEBUG:
                log.info(f"Allowing recomputation for MLP-related node: {node}")
            return False
        
        # If this is likely attention, ban recomputation
        if is_likely_attention_node(node):
            if AOT_PARTITIONER_DEBUG:
                log.info(f"Banning recomputation for attention node: {node}")
            return True
        
        # For nodes between MLP and attention, make smart decisions
        # Check if node is on critical path between marked nodes
        has_mlp_input = any(isinstance(arg, fx.Node) and is_likely_mlp_node(arg) 
                           for arg in node.args)
        has_attention_user = any(is_likely_attention_node(user) for user in node.users)
        
        if has_mlp_input and has_attention_user:
            # This is a bridge node between MLP and attention
            # Save it to avoid recomputing MLP multiple times
            return True
    ...
    def get_node_weight(node) -> float:
        ...
        # ===== Enhanced weight calculation based on marked nodes =====
        
        # Respect explicit policies
        if node.meta.get("recompute") == CheckpointPolicy.MUST_SAVE:
            # Very high weight to ensure it's saved
            return mem_sz * 1000
        
        if node.meta.get("recompute") == CheckpointPolicy.MUST_RECOMPUTE:
            # Very low weight to encourage recomputation
            return max(1, mem_sz // 100)
        
        # Smart weight based on node patterns
        if is_likely_mlp_node(node):
            # MLP nodes: low weight to encourage recomputation
            # MLP is memory-heavy but computationally cheap to recompute
            base_weight = mem_sz * 0.1
            
            # Adjust based on distance from backward
            # Closer MLP nodes are cheaper to recompute
            if node.dist_from_bw < 10:
                base_weight *= 0.5
            elif node.dist_from_bw > 100:
                base_weight *= 2.0
                
            return max(1, int(base_weight))
        
        if is_likely_attention_node(node):
            # Attention nodes: high weight to discourage recomputation
            # Attention is computationally expensive (O(n²))
            base_weight = mem_sz * 10
            
            # Further increase weight based on tensor dimensions
            if "val" in node.meta and isinstance(node.meta["val"], torch.Tensor):
                val = node.meta["val"]
                # Check for large sequence dimensions
                if val.dim() >= 2:
                    seq_dim = max(hint_int(val.shape[-2], fallback=1),
                                 hint_int(val.shape[-1], fallback=1))
                    if seq_dim > 1024:
                        # Quadratic scaling for long sequences
                        base_weight *= (seq_dim / 512) ** 2
            
            return int(base_weight)
        
        # For other nodes, use enhanced heuristic
        # Use the enhanced distance that considers marked nodes
        enhanced_dist = compute_enhanced_distance(node)
        distance_factor = 1.1 ** max(min(enhanced_dist, 100), 1)
        
        # Check proximity to marked recompute nodes
        has_recompute_neighbors = any(
            user in marked_recompute_nodes for user in node.users
        ) or any(
            isinstance(arg, fx.Node) and arg in marked_recompute_nodes 
            for arg in node.args
        )
        
        if has_recompute_neighbors:
            # Nodes near recomputed nodes should also be considered for recomputation
            distance_factor *= 0.5
        
        # Additional factor based on operation type
        target = get_aten_target(node)
        if target in [aten.mm.default, aten.addmm.default]:
            # Matrix multiplication in unmarked context
            # Could be either attention or MLP - check size
            if "val" in node.meta and isinstance(node.meta["val"], torch.Tensor):
                output_size = hint_int(node.meta["val"].numel(), fallback=1)
                if output_size > 10_000_000:  # Large intermediate activation
                    # Likely MLP intermediate, prefer recomputation
                    distance_factor *= 0.3
        
        mem_sz = int(mem_sz * distance_factor)

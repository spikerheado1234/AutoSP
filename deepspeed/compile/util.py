...
def get_sdpa_nodes(gm: GraphModule) -> List[Node]:
    """Find all scaled_dot_product_attention nodes."""
...
def create_symbolic_slice_indices(
    ...
    """
    Create FX nodes for symbolic slice indices.
    
    Returns:
        Tuple of (slice_dim0, slice_dim1) nodes representing [:, start:end]
    """
    start_node, end_node = create_shard_offsets(gm, s0_node, config.world_size, config.rank)
...
def shard_tensor_node(
    ...
    """
    Shard a 2D tensor node along the sequence dimension (dim 1).
    
    Creates: tensor[:, rank*chunk_size : (rank+1)*chunk_size]
    """

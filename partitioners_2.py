...
def solve_min_cut_sequence_aware(
    joint_graph: fx.Graph,
    node_info: NodeInfo,
    ...
    """
    Sequence-aware minimum cut algorithm optimized for long sequence training.
    
    Args:
        joint_graph: The joint forward-backward computation graph
        node_info: Information about node classifications
        min_cut_options: Configuration for minimum cut constraints
        dont_ban: Set of nodes that should not be banned from recomputation
        sequence_config: Configuration for sequence-aware optimizations
    """
    if dont_ban is None:
        dont_ban = set()
    ...
    def should_ban_recomputation_sequence_aware(node):
        ...
        # Get sequence information for enhanced decision making
        seq_length = get_sequence_length(node)
        mem_sz = _size_of(node)
        
        # Sequence-aware allowlist checking
        if min_cut_options.ban_if_not_in_allowlist:
            if not op_types.is_recomputable(node):
                # For long sequences, be more lenient with recomputation
                if seq_length > 2048 and not is_extremely_expensive_for_long_seq(node):
                    return False
                return True
        else:
            # For long sequences, be more aggressive about recomputing expensive ops
            if seq_length > 1024:
                return is_extremely_expensive_for_long_seq(node)
            else:
                return op_types.is_random(node) or op_types.is_compute_intensive(node)

        # Backward materialization check - less strict for long sequences
        if min_cut_options.ban_if_materialized_backward and is_materialized_backwards(node):
            # For very long sequences, allow recomputation even if materialized backwards
            if seq_length > 4096 and is_attention_related_node(node, seq_length):
        ...
        # Enhanced distance check for sequences
        if node.dist_from_bw < 1000 and node.dist_from_bw > config.max_dist_from_bw:
            # For long sequences, distance is less critical
            if seq_length <= 1024:
                return True
            return False

        # Sequence-aware reduction check
        if min_cut_options.ban_if_reduction:
            ...
            # For long sequences, be more lenient with reductions
    ...
    def is_extremely_expensive_for_long_seq(node) -> bool:
        """Check if an operation is extremely expensive for long sequences"""
        # Operations that scale very poorly with sequence length
        expensive_ops = [
            aten.convolution,  # Still expensive
            aten._scaled_mm,   # Matrix multiplication variants
        ]
        
        # Allow recomputation of normalization and activation ops for long sequences
        cheap_for_long_seq = [
            aten.layer_norm,
            aten.group_norm, 
            aten.batch_norm,
            ...
            aten.gelu,
            aten.silu,
            aten.softmax,  # Surprisingly cheap to recompute vs memory
        ]
        
        if get_aten_target(node) in cheap_for_long_seq:
            return False
            
        return get_aten_target(node) in expensive_ops
    
    # Use the enhanced function
    should_ban_recomputation = should_ban_recomputation_sequence_aware
    ...
    def get_sequence_aware_node_weight(node) -> float:
        """Sequence-aware weight calculation for long sequence training"""
        mem_sz = _size_of(node)
        ...
        seq_length = get_sequence_length(node)
        batch_size = get_batch_size(node)
        
        # Sequence-aware memory scaling
        sequence_factor = calculate_sequence_memory_factor(seq_length, mem_sz)
        
        # Enhanced distance calculation for long sequences
        enhanced_distance_factor = calculate_enhanced_distance_factor(
            node.dist_from_bw, seq_length
        )
        
        # Apply sequence-aware scaling
        adjusted_mem_sz = int(mem_sz * sequence_factor * enhanced_distance_factor)
        
        # Attention-specific handling
        if is_attention_related_node(node, seq_length):
            attention_penalty = calculate_attention_penalty(seq_length, mem_sz)
            adjusted_mem_sz = int(adjusted_mem_sz * attention_penalty)
        
        if is_materialized(node):
            return adjusted_mem_sz
        else:
            # Reduce penalty for non-materialized nodes in long sequences
            materialization_factor = 1.5 if seq_length > 2048 else 2.0
            return adjusted_mem_sz * materialization_factor
    ...
    def get_sequence_length(node) -> int:
        ...
        if isinstance(val, torch.Tensor) and val.dim() >= 2:
            # Assume sequence dimension is typically the second dimension
            seq_dim = hint_int(val.shape[1], fallback=512)
            return min(seq_dim, 32768)  # Cap at reasonable maximum
        return 512
    
    def get_batch_size(node) -> int:
        """Extract batch size from node metadata"""
        if "val" not in node.meta:
            return 1
        
        val = node.meta["val"]
        if isinstance(val, torch.Tensor) and val.dim() >= 1:
            return hint_int(val.shape[0], fallback=1)
        return 1
    
    def calculate_sequence_memory_factor(seq_length: int, mem_sz: int) -> float:
        """Calculate memory scaling factor based on sequence length"""
        if seq_length <= 512:
            return 1.0
        elif seq_length <= 2048:
            # Moderate scaling for medium sequences
            return 1.0 + (seq_length / 2048) * 0.5
        else:
            # Aggressive scaling for long sequences - quadratic penalty
            base_factor = 1.5
            length_factor = (seq_length / 2048) ** 1.2
            memory_factor = max(1.0, math.log10(mem_sz / 1024 / 1024 + 1))  # Log of MB
            return base_factor * length_factor * memory_factor
    
    def calculate_enhanced_distance_factor(dist_from_bw: int, seq_length: int) -> float:
        """Enhanced distance factor that considers sequence length"""
        if seq_length <= 512:
            # Standard distance calculation for short sequences
            return 1.1 ** max(min(dist_from_bw, 100), 1)
        else:
            # For long sequences, distance becomes less important
            # Focus more on memory size than temporal distance
            distance_dampening = min(2.0, seq_length / 1024)
            effective_distance = max(1, dist_from_bw / distance_dampening)
            return 1.05 ** max(min(effective_distance, 50), 1)
    
    def is_attention_related_node(node, seq_length: int) -> bool:
        """Identify attention-related nodes that scale quadratically with sequence length"""
        if hasattr(node.target, '__name__'):
            target_name = str(node.target.__name__).lower()
            attention_ops = ['scaled_dot_product', 'bmm', 'softmax', 'attention']
            is_attention = any(op in target_name for op in attention_ops)
            
            # Also check for large matrices that could be attention scores
            if seq_length > 1024 and "val" in node.meta:
                val = node.meta["val"]
                if isinstance(val, torch.Tensor) and val.dim() >= 2:
                    # Check if it's likely an attention matrix (seq_len x seq_len)
                    h, w = val.shape[-2], val.shape[-1]
                    h_hint = hint_int(h, fallback=0)
                    w_hint = hint_int(w, fallback=0)
                    if abs(h_hint - w_hint) < max(h_hint, w_hint) * 0.1:  # Square-ish matrix
                        is_attention = True
            
            return is_attention
        return False
    
    def calculate_attention_penalty(seq_length: int, mem_sz: int) -> float:
        """Calculate penalty for attention nodes based on sequence length"""
        if seq_length <= 512:
            return 1.0
        elif seq_length <= 2048:
            return 2.0  # Moderate penalty
        else:
            # Heavy penalty for long sequence attention
            length_penalty = (seq_length / 2048) ** 1.5
            size_penalty = max(1.0, math.log10(mem_sz / (1024 * 1024) + 1))
            return min(10.0, 3.0 * length_penalty * size_penalty)
    
    # Use the new function instead of the old one
    get_node_weight = get_sequence_aware_node_weight
...
# Backward compatibility - use sequence-aware version as default
def solve_min_cut(
    joint_graph: fx.Graph,
    node_info: NodeInfo,
    min_cut_options: MinCutOptions,
    ...
    """Backward compatibility wrapper for solve_min_cut_sequence_aware"""
    return solve_min_cut_sequence_aware(
        joint_graph, node_info, min_cut_options, dont_ban
    )
...
def choose_saved_values_set(
    ...
    runtime_optimized_saved_values, _ = solve_min_cut_sequence_aware(
        joint_graph,
        node_info,
        min_cut_options,
    )
    ...
    more_aggressive_saved_values, _ = solve_min_cut_sequence_aware(
    ...
    aggressive_recomputation_saved_values, banned_nodes = solve_min_cut_sequence_aware(
    ...
    def get_saved_values_knapsack(memory_budget):
        ...
        saved_values, _ = solve_min_cut_sequence_aware(
            joint_graph,
            node_info,
            aggressive_options,
            dont_ban,
